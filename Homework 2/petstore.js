...
function calculateFoodOrder(numAnimals, avgFood) {
    ...
    // Convert arguments to numeric form
    ...
    // check that arguments are positive numbers
    if (avgFood < 0 || numAnimals < 0 || !/[0-9]/.test(avgFood) || !/[0-9]/.test(numAnimals)) {
        return -1;
    }
    else {
...
function mostPopularDays(week) {
    ...
    var counter = 0;
    var result = [week[counter].name];
    for (let i = 1; i < week.length; i++) {
        if (week[i].traffic > week[counter].traffic) {
            result[0] = week[i].name;
            counter = i;
        }
        else if (week[i].traffic === week[counter].traffic) {
