...
class FontChooser extends React.Component {
    ...
    constructor(props) {
	super(props);
	this.state = { hidden: true, bold: false, checked: false, size: this.props.size, color: 'black', min: this.props.min, max: this.props.max };	
	}
	// function to hide/unhide form
	toggleHide() {
		this.setState( { hidden: !this.state.hidden } );
	}
	// function to toggle fontweight (bold/normal) and checkbox (checked/unchecked) 
	toggleCheck() {
		this.setState( { bold: !this.state.bold } );
		this.setState( { checked: !this.state.checked } );
	}
	// function to decrease fontsize and make fontcolor red at min (black between min and max)
	fontSizeDecrease() {
		if (this.state.size > Number(this.state.min) + 1) {
			this.setState( { size: this.state.size - 1 } );
			this.setState( { color: 'black' } );
		}
		else if (this.state.size === Number(this.state.min) + 1) {
			this.setState( { size: Number(this.state.size) - 1} );
			this.setState( { color: 'red' } );
		}	
	}
	// function to increase fontsize and make fontcolor red at max (black between min and max)
	fontSizeIncrease() {
		if (this.state.size < Number(this.state.max) - 1) {
			this.setState( { size: Number(this.state.size) + 1 } );
			this.setState( { color: 'black' } );
		}
		else if (this.state.size === Number(this.state.max) - 1) {
			this.setState( { size: Number(this.state.size) + 1 } );
			this.setState( { color: 'red' } );
		}
	}
	// function to change fontsize to default
	doubleClick() {
	...
	errorHandler() {
		...
		// If the “min” property is greater than the “max” property, then “min” and “max” should both be treated as the larger of the two, i.e. the “min” property
		else if (Number(this.props.min) > Number(this.props.max)) {
			this.setState( { min: this.props.max } );
			this.setState( { max: this.props.min } );
		}
		// If the “size” property is less than the “min” property, the initial value should be treated as the same as the “min” property
		else if (Number(this.props.size) < Number(this.props.min) && Number(this.props.min) > 0) {
			this.setState( { size: this.props.min } );
		}
		// or treated as 1 if “min” is 0 or negative
		else if (Number(this.props.size) < Number(this.props.min) && Number(this.props.min) <= 0) {
	...
	// function to set the state and check for errors when the page is loaded
	componentDidMount() {
		this.errorHandler();
